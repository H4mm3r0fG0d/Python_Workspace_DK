...
class AISTransponder:
    def __init__(self, ship, remote_host, remote_port):
        self.ship = ship
    ...
    def broadcast_position_report(self):
        report = self.message.generate_position_report()
        self.sock.sendall(report.encode('utf-8'))
...
def main():
    ...
    transponders = [AISTransponder(ship, remote_host, remote_port) for ship in ships]
